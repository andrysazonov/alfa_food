...
const NavBar: React.FunctionComponent<INavBarProps> = ({ loggedInUser }) => {
  let path1 = "/register";
  let path2="/login"

  if (loggedInUser.role !== "none") {
    
    path1 = "/accountsettigns";
    path2="/"
    ...
    <div className="nav-bar">
      ...
      <div className="block">
        <Link to={path1}>
          ...
          {loggedInUser.role !== "none" ? "username" : "Зарегистрироваться"}
        </Link>

        <Link to={path2}>{loggedInUser.role !== "none" ? "Выйти" : "Войти"}</Link>
      </div>
    </div>
  );
};
